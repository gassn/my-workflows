...
#### Phase 5 (2026-04-23 実装、agent 多階層制約で設計転換)

| 実装済 | 対応 | 備考 |
|---|---|---|
| ✅ orchestrator **skill** (当初 agent → skill 転換) | main agent | Claude Code「Subagents cannot spawn their own subagents」の制約により agent 3 階層不可と判明、main agent が orchestrator skill + spec-leader skill を兼任する設計に転換 |
| ✅ investigator agent | writing-plan / brainstorming | 3 responsibility (codebase / other-plans / dependencies) を並列起動 |
| ✅ spec-reviewer agent × 3 並列 | spec-review | 完全性 / 実現可能性 / 整合性 の独立並列判定 |
| ✅ WorktreeCreate hook | (worktree 管理) | Agent isolation / EnterWorktree 作成時に Spec/Plan/Review 自動コピー + progress.md 初期化 |
| ✅ WorktreeRemove hook | (worktree 管理) | 削除前の未コミット / archive 未完警告 + progress.md バックアップ |
| ✅ TaskCompleted hook | (Task 系連携) | Claude Code TaskUpdate status=completed で progress.md 自動追記 |
...
| マルチセッション並列化 | 各 Spec を独立 Claude Code セッションで並列、main session が統括 |
| hookify 実有効化 | learn.md が 10-20 Spec 分蓄積後、パターンから hook 一括生成 |

詳細は `docs/phase4-completion.md` / `docs/phase5-completion.md` および `ROADMAP.md` 参照。
