...
Spec 名は **kebab-case (`^[A-Za-z0-9._-]+$`)** に統一してください。`tmux-dashboard-mvp` / `ecsite-mvp-auth` / `api-refactor-phase2` のような形です。スペース / 日本語 / 特殊文字を含むと、tmux-dashboard の allowlist や git ブランチ名制約に引っかかります。
