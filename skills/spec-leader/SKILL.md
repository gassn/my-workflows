...
3. main 側の 3 ファイルを worktree 内にコピー:
   - `specs/<spec-name>.md` → `worktrees/<spec-name>/specs/<spec-name>.md`
   - `specs/<spec-name>.plan.md` → `worktrees/<spec-name>/plans/<spec-name>.md` (**worktree 側では従来通り `plans/` サブディレクトリ命名**)
   - `specs/<spec-name>.review.md` → `worktrees/<spec-name>/specs/<spec-name>.review.md` (参考情報)
...
6. **Spec / Plan / Review を archive 移動** (2026-04-22 改修):
   ...
   - archive 移動により、過去の設計判断 / レビュー結果 / Plan を将来の Spec 策定時に参照可能
