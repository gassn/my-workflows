...
- **各 iteration の Critical / Major / Minor 件数推移**: iteration 1 の指摘数 → iteration 2 で何件解消 / 何件新規発生 / 何件見送り、を `consolidated.md` の iteration_N 記録から抽出して表化
- **解消 vs 見送り比率**: 各 severity ごとに `対応 / (対応 + 見送り)` を算出。Critical は 100% 必須、Major は 80%+ を想定下限、Minor は 50%+ を目安とし、下回った場合は §4 Problem に「指摘対応率の低下」として記録
- **loop 収束ステップ数**: iteration 1 の reject → iteration N で全 pass に至るまでのステップ数。3 回上限 (receiving-code-review §4 循環防止) 近辺は要警戒
