...
- **Phase 3 (skill + agent 実装 + eval)**: ✅ 完了 (12 skill / 7 agent、iter-3/4/5 統合完走 shipped + Phase 6 バッチ 2 (a) `tmux-dashboard-mvp` で再検証済)
- **Phase 4 (hook 自動化)**: ✅ 完了 (8 hook 実装 + hookify 方針確定)
- **Phase 5 (orchestrator + 並列実行)**: ✅ 実質完了 (orchestrator skill + 3 hook + 2 agent、Agent Teams 多階層禁止対応込み)
- **Phase 6 (統合改善 + 公開検討)**: 🚧 バッチ 1 完了 / バッチ 2 (a) 完了 / バッチ 2 (b) 以降と LLM 定量 eval が残
  - バッチ 1 完了 7 項目: CLAUDE.md 体系化 / memory 運用 / ドキュメント整備 / MIT License 採択 / ベストプラクティス集 / GitHub 公開 (https://github.com/gassn/my-workflows) / skill-creator 記述ベース実証 3 skill
  - バッチ 2 (a): `tmux-dashboard-mvp` shipped-cross-model-pending、learn Try 5.1 / 5.3 / 5.4 / 5.5 を 3 skill に反映済

詳細は各 Phase 完了レポート (`docs/phase3-completion.md` / `docs/phase4-completion.md` / `docs/phase5-completion.md` / `docs/phase6-progress.md`) と `ROADMAP.md` を参照してください。
...
├── tools/                             # ドッグフーディング成果物 (tmux ダッシュボード)
├── tests/                             # tools/ のテスト (bash 構文 + エラーパス)
├── specs/                             # Spec 作業領域
│   └── archive/                       # ship 済 Spec の集約 (spec/plan/review/progress/result/consolidated/learn)
├── docs/
...
│   ├── tmux-dashboard-operation.md    # tmux ダッシュボード運用ガイド
...
│   └── phase6-progress.md             # Phase 6 進捗レポート (完了時に phase6-completion.md へ改名)
...
2. **docs/phase<N>-completion.md** (N=3,4,5) / **docs/phase6-progress.md**: 各 Phase の完了レポート / Phase 6 は進捗レポート (実装詳細 + 動作検証 + 知見)
3. **docs/best-practices.md**: 12 skill の利用例とハマりどころ集、他プロジェクト持ち込みガイド
4. **docs/components-map.md**: skill / agent / hook の現在地
5. **docs/workflow.md**: ワークフロー定義
6. **specs/archive/**: ship 済 Spec の成果物 (learn.md 含む、ドッグフーディング知見の蓄積先)
